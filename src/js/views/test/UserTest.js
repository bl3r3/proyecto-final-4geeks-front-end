...
export const UserTest = props => {
  ...
  const renderBtn = () => {
    if (formStep > 5) {
      ...
      return (
        <button disabled={!isValid} type="submit" className="btn btn-radius btn-info">
          Finalizar Test
        </button>
      );
    } else {
      return (
        <div className="btn-wizard">
          {formStep === 0 ? null : formStep === 1 ? null : (
            <button disabled={!isValid} onClick={backTetsStep} type="button" className="btn-radius btn btn-warning">
              Regresar
            </button>
          )}
          <button disabled={!isValid} onClick={completedTetsStep} type="button" className="btn btn-success btn-radius">
            ...
            {formStep >= 0 && (
              <section className={formStep === 0 ? 'test-active' : 'd-none'}>
                <div className="w-100">
                  ...
                  <h3>
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed dictum condimentum sodales. Class
                    aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Donec ultrices
                    tincidunt blandit. Phasellus nec nunc molestie sapien eleifend consequat vel ut nisi.
                  </h3>
                </div>
              </section>
            )}
